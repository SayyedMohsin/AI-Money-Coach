...
# Helper Function: Monthly calculation aur Rollover ke sath
def get_financial_summary():
    ...
    # 1. Pichle mahine tak ka bacha hua paisa (Rollover / Carry Forward)
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Income' AND date < ?", (first_day_of_month,))
    past_income = cursor.fetchone()[0] or 0.0
    
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Expense' AND date < ?", (first_day_of_month,))
    past_expense = cursor.fetchone()[0] or 0.0
    
    carry_forward_balance = past_income - past_expense

    # 2. Is mahine ki aamdani aur kharcha
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Income' AND date LIKE ?", (f"{current_month_prefix}%",))
    monthly_income = cursor.fetchone()[0] or 0.0
    
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Expense' AND date LIKE ?", (f"{current_month_prefix}%",))
    monthly_expense = cursor.fetchone()[0] or 0.0
    
    # Is mahine ke unique days (average nikalne ke liye)
    cursor.execute("SELECT COUNT(DISTINCT date) FROM transactions WHERE type='Expense' AND date LIKE ?", (f"{current_month_prefix}%",))
    unique_days = cursor.fetchone()[0] or 1
    
    conn.close()
    
    # Total Balance = Purana Bacha Hua + Nayi Aamdani - Naya Kharcha
    current_balance = carry_forward_balance + monthly_income - monthly_expense
    
    # Roz ka kharcha (sirf is mahine ka)
    ...
    return {
        ...
        "total_expense": monthly_expense, # UI compatibility ke liye isko total_expense rakha hai
...
def add_transaction(t: Transaction):
    conn = sqlite3.connect("finance_data.db")
    cursor = conn.cursor()
    # Naya date logic: User ki di hui date use karein, warna aaj ki date lein
    ...
    cursor.execute(
        ...
        (selected_date, t.type, t.category, t.amount, t.note)
    )
    conn.commit()
    conn.close()
...
# NAYA ENDPOINT: Kisi bhi mahine ka Invoice/Report nikalne ke liye
@app.get("/monthly-report/{year_month}")
def get_monthly_report(year_month: str):
    # year_month format: "YYYY-MM" (e.g., "2026-04")
    conn = sqlite3.connect("finance_data.db")
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    
    cursor.execute("SELECT * FROM transactions WHERE date LIKE ? ORDER BY date ASC", (f"{year_month}%",))
    transactions = cursor.fetchall()
    
    cursor.execute("SELECT category, SUM(amount) as total FROM transactions WHERE type='Expense' AND date LIKE ? GROUP BY category", (f"{year_month}%",))
    categories = cursor.fetchall()
    
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Expense' AND date LIKE ?", (f"{year_month}%",))
    total_exp = cursor.fetchone()[0] or 0.0
    
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE type='Income' AND date LIKE ?", (f"{year_month}%",))
...
# NAYA ENDPOINT: CSV (Excel) Download karne ke liye
@app.get("/download-csv/{year_month}")
def download_csv(year_month: str):
    conn = sqlite3.connect("finance_data.db")
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT date, type, category, amount, note FROM transactions WHERE date LIKE ? ORDER BY date ASC", (f"{year_month}%",))
    transactions = cursor.fetchall()
    conn.close()
...
def chat_with_ai(request: ChatRequest):
    try:
        ...
        # Is mahine ki category breakdown AI ko bhejne ke liye
        conn = sqlite3.connect("finance_data.db")
        cursor = conn.cursor()
        current_month = datetime.date.today().strftime("%Y-%m")
        cursor.execute("SELECT category, SUM(amount) FROM transactions WHERE type='Expense' AND date LIKE ? GROUP BY category", (f"{current_month}%",))
        categories = cursor.fetchall()
        conn.close()
...
# NAYA ENDPOINT: Database ko clear karne ke liye
@app.delete("/clear-db")
def clear_database():
    conn = sqlite3.connect("finance_data.db")
    cursor = conn.cursor()
    cursor.execute("DELETE FROM transactions")
    # Auto-increment ID ko bhi reset karne ke liye
    cursor.execute("DELETE FROM sqlite_sequence WHERE name='transactions'")
    conn.commit()
    conn.close()
    return {"message": "Saara data successfully delete ho gaya hai!"}